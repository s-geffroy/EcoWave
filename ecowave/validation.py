...
def check_config(settings: Settings, mode: str) -> CheckResult:
    ...
    db_ok = schema_version == "0.1.0"
    if mode == "strict":
        items.append(CheckItem("SQLite schema", db_ok, f"expected 0.1.0, found {schema_version}"))
    else:
        items.append(CheckItem("SQLite schema", True if schema_version in {None, "0.1.0"} else False, f"found {schema_version}; run init-db if missing"))
