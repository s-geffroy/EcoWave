...
def _run_bis(settings: Settings, as_of: str, manifest_path: Path,
             groups: list[str], mode: str,
             n_surrogates: int, seed: int,
             con: "sqlite3.Connection", run_id: int,
             null: str = "ar1") -> Path:
    """BIS bulk macroprudential variant. Roadmap #13 Phase 2.

    Brings emerging markets onto the CPV panel (entirely absent from
    prior horizons) via quarterly BIS series — credit-to-GDP gap (Borio
    CCyB), credit ratio, total credit, residential property prices.
    Reuses the quarterly machinery (``samples_per_year=4``).
    """
    from ecowave.cycles.bis_bulk import (
        BIS_GROUPS,
        BisBulkDataset,
        build_bis_panel,
        load_bis_manifest,
    ...
    spec = load_bis_manifest(Path(manifest_path))
    samples_per_year = float(spec.get("samples_per_year", 4))
    variable_specs = spec.get("variable_codes", [])
    start_year = int(spec.get("start_year", 1970))
    typer.echo(
        f"CPV (BIS bulk) — {spec.get('project','cpv_bis_macroprudential')}; "
        f"{len(variable_specs)} variables; {len(groups)} groups; "
        f"start_year={start_year}; samples_per_year={int(samples_per_year)}."
    )

    dataset = BisBulkDataset.default(settings.data_raw_dir)
    composite_by_group: dict[str, pd.Series] = {}
    panels_by_group: dict[str, pd.DataFrame] = {}
    for group in groups:
        if group not in BIS_GROUPS:
            typer.echo(f"  BIS group {group} unknown — skipped.", err=True)
            continue
        try:
            panel = build_bis_panel(group, variable_specs, dataset,
                                     start_year=start_year,
                                     persist={"con": con})
        except Exception as exc:  # noqa: BLE001
            typer.echo(f"  Group {group}: BIS ingestion failed: {exc}",
                       err=True)
            if mode == "strict":
                finish_ingestion_run(con, run_id, "failed", notes=str(exc))
                con.close()
            ...
            continue
        if panel.empty:
            typer.echo(f"  Group {group}: empty BIS panel — skipped.")
            continue
        panels_by_group[group] = panel
        composite_by_group[group] = _composite_panel(panel,
                                                       samples_per_year=samples_per_year)
    ...
    if not composite_by_group:
        typer.echo("No BIS group produced data; aborting.", err=True)
        finish_ingestion_run(con, run_id, "failed", notes="empty panels")
        con.close()
        raise typer.Exit(code=1)
    ...
    return _analyse_and_render(
        ...
        panels_by_group=panels_by_group,
        n_surrogates=n_surrogates, seed=seed, null=null,
        horizon_label=f"BIS macroprudential ({start_year}-2025 quarterly)",
