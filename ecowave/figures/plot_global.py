...
def plot_global_indices(global_indices: pd.DataFrame, output_path: Path, pilot: str,
                        ref: str = "precrisis", waves: pd.DataFrame | None = None) -> Path:
    """Plot intensity (3 weightings) + diffusion histogram for a single ref window."""
    ...
    for weighting, style in WEIGHTING_STYLE.items():
        ...
        if wsub.empty:
            continue
        # Use the MA3-smoothed intensity for the Elliott-relevant view.
        ax_top.plot(
            range(len(months)),
            [wsub.loc[m, "intensity_ma3"] if m in wsub.index else float("nan") for m in months],
            linestyle=style["linestyle"], color=style["color"], linewidth=1.6,
            label=f"I_intensity ({style['label']})",
        )

    ax_top.axhline(75, color="grey", linestyle="--", linewidth=0.8, label="high-stress threshold (75)")
    ax_top.set_ylabel("I_intensity (MA3, 0-100)")
    ax_top.set_ylim(0, 105)
    ax_top.set_title(f"EcoWave — synthetic indicators, pilot {pilot} (ref={ref})")
    ax_top.grid(True, alpha=0.25)

    # Annotate Elliott waves (drawn for the 'equal' / ma3 variant if present).
    if waves is not None and not waves.empty:
        equal_ma3 = waves[(waves["weighting"] == "equal") & (waves["smoothing"] == "ma3")]
        for w in equal_ma3.itertuples():
            x = pos.get(w.end_month)
            if x is None:
                continue
            color = "darkgreen" if w.confirmed else "grey"
            ax_top.annotate(
                w.label, xy=(x, w.end_value),
                xytext=(0, 6), textcoords="offset points",
                fontsize=9, color=color, ha="center", weight="bold",
            )

    # Diffusion = how many curves above the 80th percentile this month (equal across weightings).
    ...
    ax_bot.axhline(3, color="#cc5500", linestyle="--", linewidth=0.8, label="diffusion confirmation (≥3)")
