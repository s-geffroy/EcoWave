...
Table~\ref{tab:power} reports \(\hat\pi\) for each combination.

\begin{table}[h]
\centering
\caption{Empirical detection power \(\hat\pi\) at \(\alpha=0.05\)
on \(T=150\) annual observations, \(N=1000\) replicates.}
\label{tab:power}
\small
...
\cmidrule(l){2-6}
   & 0.2 & 0.3 & 0.5 & 1.0 & 2.0 \\
\midrule
Family I — AR(1) + cosine    & 0.18 & 0.42 & 0.74 & 0.93 & 0.99 \\
Family II — MS-AR(1)          & 0.11 & 0.28 & 0.56 & 0.81 & 0.95 \\
Family III — MFRW + modulation & 0.07 & 0.19 & 0.43 & 0.71 & 0.88 \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Implication.}
The protocol has acceptable power above
\(\mathrm{SNR}\approx 0.5\) on AR(1)+cosine, but degrades
substantially on more realistic generative models. Below
\(\mathrm{SNR}\approx 0.3\) the protocol effectively cannot
detect cycles, and macroeconomic data with a small cyclical share
(typical estimates of the cyclical share of post-1945 US GDP put
it at 10--20\,\% \citep{king_plosser_stock_watson1991}) sit at
or below this boundary. A ``rejected'' verdict in
Section~\ref{sec:results} therefore should be read as ``not
detected under the present protocol at the SNR available on
this panel''. We make this disclosure load-bearing rather than
incidental.
